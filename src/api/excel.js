...
export const downloadExcel = async (id) => {
  ...
  const response = await axiosInstance.get(`${apiUrl}/admin/event/excel/download/${id}`, {
...
export const uploadExcel = async (id, file) => {
  ...
  const response = await axiosInstance.post(`${apiUrl}/admin/event/excel/upload/${id}`, formData, {
    headers: {
